...
def train_step(
    ...
    def loss_fn(
        ...
        if hasattr(model, "compute_loss_terms"):
            terms = model.compute_loss_terms(rng, observation, actions, train=True)
            scalar_terms = {key: jnp.mean(value) for key, value in terms.items()}
            return scalar_terms["loss"], scalar_terms
